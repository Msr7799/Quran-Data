...
│   │   └── timingController.mjs         # مسؤول عن معالجة طلبات التوقيت
│   ├── 📂 middleware
...
│   ├── 📂 public
│   │   ├── docs.html                    # صفحة توثيق API الرئيسية
│   │   ├── style.css                    # ملف التنسيق
│   │   ├── Quran-Data.png               # صورة المشروع
│   │   └── icon-192.png                 # أيقونة المشروع
│   ├── config.mjs                       # إعدادات التكوين الخاصة بالتطبيق
│   └── server.mjs                       # نقطة الدخول للتطبيق وتشغيل الخادم
│
├── 📂 data
...
│   │   ├── api_reference.json           # مرجع API شامل للتوثيق
│   │   ├── 📂 surah
│   │   │   └── surah_*.json             # بيانات السور بشكل مفصل
│   │   ├── 📂 verses
│   │   │   └── 001_001.json             # بيانات الآيات لكل سورة
│   │   ├── 📂 audio
│   │   │   └── audio_surah_*.json       # بيانات الصوتيات لكل سورة
│   │   └── 📂 ayat_Timming
│   │       ├── sudais.json              # توقيت آيات السديس
│   │       ├── Shuraym.json             # توقيت آيات الشريم
│   │       ├── Hudhaify.json            # توقيت آيات الحذيفي
│   │       └── alafasy.json             # توقيت آيات العفاسي
│   ├── 📂 sqlite
│   │   └── database.sqlite              # قاعدة البيانات بصيغة SQLite
│   ├── 📂 csv
│   │   └── database.csv                 # قاعدة البيانات بصيغة CSV
│   └── 📂 quran_image
│       └── *.png                        # صور صفحات القرآن (604 صفحة)
│
├── 📂 scripts
│   ├── jsonToSqlite.mjs                 # إنشاء قاعدة البيانات الأساسية
│   ├── addTimingToSqlite.mjs            # إضافة بيانات التوقيت + دوال مساعدة
│   ├── addApiReferenceToSqlite.mjs      # إضافة مرجع API إلى قاعدة البيانات
│   ├── addJsonToSqlite.mjs              # إدخال ملفات JSON عامة
│   ├── splitData.mjs                    # تقسيم البيانات إلى ملفات منفصلة
│   ├── jsonToCsv.mjs                    # تحويل إلى CSV
│   ├── fetchJson.mjs                    # دالة جلب JSON من الإنترنت
│   ├── translateText.mjs                # ترجمة النصوص
│   └── README.md                        # توثيق السكربتات
│
├── 📂 docs
│   └── api-definition.yaml              # تعريف API بصيغة OpenAPI
│
└── 📄 README.md
```
...
3. **تشغيل السكربتات:**
   ...
   للعمل مع البيانات وقاعدة البيانات، استخدم الأوامر المختصرة التالية:

   ```bash
   # إعداد قاعدة البيانات كاملة (موصى به للمبتدئين)
   yarn setup:db
   # أو
   npm run setup:db
   ```
   ...
   **أوامر السكربتات الفردية:**
   ...
   # إضافة بيانات التوقيت
   yarn addTimingToSqlite
   
   # إضافة مرجع API
   yarn addApiReferenceToSqlite
   
   # تحويل إلى CSV
   yarn jsonToCsv
   
   # تقسيم البيانات إلى ملفات منفصلة
   yarn splitData
   ```
...
- **4** قراء مع بيانات توقيت دقيقة (السديس، الشريم، الحذيفي، العفاسي)
...
- دعم التوقيت المتقدم للقراء
- صور صفحات القرآن
- بحث بالسورة والآية والجزء
- معلومات السجدة والصفحات
- مرجع API شامل في قاعدة البيانات
